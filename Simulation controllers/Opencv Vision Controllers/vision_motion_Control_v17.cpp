...
//Box handling
bool oneBoxDown = true;                                    /// Chnage this for second box
bool overRideBrokenBridge = true;
...
double segmentedBaseSpeed = 7.0;
double segmentedLeftSpeed;  
double segmentedRightSpeed; 
bool hardWireWallLeft = false;
/*int wall_speed=7;
double error,previous_error1=0,previous_error2=0,integral1,integral2,derivative1,derivative2,PID_error1,PID_error2;
  ...
  //Second box box arena
  int secondBoxCounter = 0;
    ...
    if (location=="Line_Following"){
        ...
        rightSpeed = speedControl(wallSpeed-lineControlSignal);
            ...
            if(floorLocation=="T_Junction"){   //Decision at first junction
             ...
             if(oneBoxDown){
                int valSum = sensorReadings[0]+sensorReadings[1]+sensorReadings[2]+sensorReadings[3]+sensorReadings[4]+sensorReadings[5]+sensorReadings[6];
                if(valSum == 0){
                  secondBoxCounter++;
                  if(secondBoxCounter>10){
                    location = "Box_Arena";
                    secondBoxCounter = 0;
                    cout<<"Location changed to box_Arena"<<endl;


                    //reset all the variables 
                    boxSearch = true;
                    searchCounter = 0;
                    //xBoxPoints[5]= {0,0,0,0,0};  //Sorted x points of the box
                    //xPointUnsorted[5]= {0,0,0,0,0};  //unsorted xpoints

                    //Box arena variable
                    sendForward = true;   //send the robot forward for some time to reduce errors
                    firstHop = false;
                    initiateHop = true;
                    searchRange = 5;
                    leftWallCounter = 0;
                    ...
                    rampPosition = "none";
                    armPosition = false;
                    ///imageSensorReadings[32];
                    //imageSensorLineY = 50;
                    //int followingXPoint = 42;
                    brokenBridgeError = 0;
                    horizontalSurface = false;
                    horizontalCounter = 0;
                    //int countToHoles = 0;
                    holeDetected = false;
                    reachedHole = false;
    ...
    if ((xBoxPoints[4] <31- searchRange) and not reached and (not firstHop)){ //Have to turn Left
      ...
      if((reached and (leftWall or rightWall)) and (not orientation)){
       ...
       if ((sonicSensorReadings[1]>sonicSensorReadings[2] +10) and reached and (not orientation)){  //check side sensors with 10 gap to get correct orientation
         ...
         if((stage>15) and (stage < 60)){
          //cout<<"stage 5"<<endl;
          reverse();
          stage++;
         } 
         if((stage>=60) and (stage <100)){
          turnRight();
          stage++;
         }
         if(stage>=100){
         ...
         if(stage == 0){        //Stage 01
          ...
          if(stage==1){   //Stage 2 open the grippers
           /*if(catchRight <0.5 && catchLeft >-0.22){
            ...
            if(catchRight >0.2 && catchLeft <0.2){     //catchRight> 0     catchLeft <0.4
              catchRight += -0.1; 
              catchLeft -= -0.1; 
              }
            else {
...
void liftTheBox(){
      ...
      if(stage == 3){  //stage 04
            ...
            if( catchRight <0.55 && catchLeft >-0.27){   // grab the box    //catchRight <0.6 && catchLeft >-0.32
              catchRight += 0.1;
              catchLeft -= 0.1;
              }
            else {
...
void wallFollowing(){
   ...
   if(turnOverRide){
      ...
      if(wallCounter>120){
        turnOverRide = false;
        hardWireWallLeft = false;
        wallCounter = 0;
      }
      if ((segmentedRightSpeed>segmentedLeftSpeed) and (not hardWireWallLeft)){
       turnLeft();
      }
      else if((segmentedRightSpeed>segmentedLeftSpeed) and (not hardWireWallLeft)){
        turnRight();
      }
      else{     //Change this
        turnLeft();
        hardWireWallLeft = true;
        
        cout<<"Equal velocities"<<endl;
      }
   }
   else{
          if(sonicSensorReadings[0]<600){
                segmentedBaseSpeed = 0.0;
          ...
          if(segmentedLeftSpeed>10.0 ){
            segmentedLeftSpeed = 10.0;
          }
          if(segmentedLeftSpeed<-10.0 ){
            segmentedLeftSpeed = -10.0;
          } 
          if(segmentedRightSpeed>10.0 ){
            segmentedRightSpeed = 10.0;
          }
          if(segmentedRightSpeed<-10.0 ){
            segmentedRightSpeed = -10.0;
          }
